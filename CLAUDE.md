...
MVVM with **Hilt** for dependency injection. `AutoHrvApplication` is annotated `@HiltAndroidApp`; `HrvRepository` is a `@Singleton` injected via constructor; `MainViewModel` exposes `StateFlow` for UI state.

UI is built with **Jetpack Compose**. Entry point is `MainActivity` → `MainScreen`. The `ui/` package is organized per on-screen section (`ui/topbar/`, `ui/coupling/`, `ui/metrics/`, `ui/acf/`, `ui/spectrogram/`), plus `ui/commons/` for shared primitives (cards, headers, chart math) and `ui/theme/`. There is a `domain/` layer containing use cases (e.g. `BreathingPacerUseCase`, `TimeSeriesStatsUseCase`) and per-feature `*Business` classes (e.g. `BreathingBusiness`, `SpectrogramBusiness`) that compose them into `StateFlow`s the ViewModel consumes.
...
When adding a value that a `*Business`/use case class exposes, ask "is this the domain fact, or a decision about how to draw it?" — the former belongs in `domain/`, the latter in `MainViewModel`.
