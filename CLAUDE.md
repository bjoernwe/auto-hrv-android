...
`domain/` is organized per business concern — `domain/signal/` (pure DSP math, no Android/coroutines), `domain/breathing/`, `domain/metrics/`, `domain/spectral/` — each with its own `model/` (domain data classes, `*BO` postfix, e.g. `BreathingPatternBO`, `SpectrogramSliceBO`) and `usecase/` (single-action classes with `operator fun invoke`, per [Android's use case convention](https://developer.android.com/topic/architecture/domain-layer)). Each concern's `*Service` class (e.g. `BreathingService`, `SpectrogramService`, `MetricsService`) is an app-scoped `@Singleton` that reads history from `HrvRepository`, reads user-set state from a settings repository if any, composes the concern's use cases, and exposes the results to the ViewModel — as `StateFlow`s where a derivation has several consumers or must survive a config change (session-long ACF accumulation, the running pacer), and as plain `Flow`s otherwise. The `Service` name reflects that lifecycle: these live for the whole app session, independent of any ViewModel. They hold no source of truth — everything they expose is derived and rebuilds from `HrvRepository` within a window's worth of beats. User-editable settings (e.g. `BreathingSettingsRepository.targetInOutBias`) live in `data/settings/`, not in a `*Service` class, so state and its derivation stay separate; `MainViewModel` reads/writes such settings repositories directly rather than through a `*Service` class.
...
When adding a value that a `*Service`/use case class exposes, ask "is this the domain fact, or a decision about how to draw it?" — the former belongs in `domain/`, the latter in `MainViewModel` (or a UI-layer mapper, e.g. `ui/acf/AcfHistogramShaping.kt`).
