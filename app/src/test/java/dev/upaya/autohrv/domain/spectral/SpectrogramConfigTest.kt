...
class SpectrogramConfigTest {
    ...
    fun `band window must be a power of two`() {
        assertThrows(IllegalArgumentException::class.java) {
            SpectrogramBand(windowSeconds = 100, hopSeconds = 5, maxSlices = 10, freqRangeHz = 0f..0.1f)
        }
    }
    ...
    fun `default bands each resolve into multiple in-range bins`() {
        SpectrogramConfig.DEFAULT.bands.forEach { band ->
            ...
            assertTrue(
                "band ${band.freqRangeHz} should resolve several bins, got ${bins.size}",
                bins.size >= 2,
            )
        }
    }
    ...
    fun `each default band window is long enough to resolve its slowest in-range bin`() {
        SpectrogramConfig.DEFAULT.bands.forEach { band ->
            ...
            assertTrue(
                "band ${band.freqRangeHz}: slowest bin ${slowestHz}Hz below the ${band.windowSeconds}s window floor",
